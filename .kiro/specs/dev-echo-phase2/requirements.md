...
- **Reading_Mode**: The operational mode for text-to-speech playback, entered via `/read` from Command_Mode
- **TTS_Engine**: The text-to-speech engine using macOS `AVSpeechSynthesizer` for audio synthesis and playback
...
### Requirement 12: Reading Mode Entry and Exit

**User Story:** As a developer, I want to enter and exit a reading mode, so that I can have text read aloud instead of reading it manually.

#### Acceptance Criteria

1. WHEN a user enters `/read` in Command_Mode THEN THE CLI_Interface SHALL transition to Reading_Mode and display available reading commands
2. WHEN a user enters `/quit` in Reading_Mode THEN THE CLI_Interface SHALL stop any active speech and return to Command_Mode
3. WHILE in Reading_Mode THEN THE CLI_Interface SHALL display a mode indicator showing "Reading Mode"
4. WHILE in Reading_Mode THEN THE CLI_Interface SHALL only accept reading-specific commands (text input, /stop, /voice, /speed, /quit)
5. WHEN transitioning to Reading_Mode THEN THE CLI_Interface SHALL initialize the TTS_Engine with default voice and speed settings

### Requirement 13: Text-to-Speech Playback

**User Story:** As a developer, I want text I type to be read aloud, so that I can listen to content without reading it manually.

#### Acceptance Criteria

1. WHEN a user enters text without a `/` prefix in Reading_Mode THEN THE TTS_Engine SHALL synthesize and play the text as speech
2. WHEN a user enters `/stop` in Reading_Mode THEN THE TTS_Engine SHALL immediately stop the current speech playback
3. WHEN a new text is entered while speech is already playing THEN THE TTS_Engine SHALL stop the current speech and start playing the new text
4. WHEN speech playback completes THEN THE CLI_Interface SHALL display a completion indicator and restore the input prompt
5. WHILE speech is playing THEN THE CLI_Interface SHALL display a playback indicator with elapsed time
6. WHEN text is empty or whitespace-only THEN THE TTS_Engine SHALL ignore the input without error

### Requirement 14: TTS Voice and Speed Control

**User Story:** As a developer, I want to control the voice and speed of text-to-speech, so that I can customize the listening experience.

#### Acceptance Criteria

1. WHEN a user enters `/voice` without arguments in Reading_Mode THEN THE CLI_Interface SHALL display the current voice and list available voices
2. WHEN a user enters `/voice {name}` in Reading_Mode THEN THE TTS_Engine SHALL switch to the specified voice for subsequent playback
3. WHEN a user enters `/speed` without arguments in Reading_Mode THEN THE CLI_Interface SHALL display the current speech rate
4. WHEN a user enters `/speed {rate}` in Reading_Mode THEN THE TTS_Engine SHALL adjust the speech rate (0.1 to 2.0, default 0.5)
5. IF an invalid voice name is specified THEN THE CLI_Interface SHALL display an error with available voice names
6. IF an invalid speed value is specified THEN THE CLI_Interface SHALL display an error with valid range (0.1-2.0)

### Requirement 15: TTS Status Display

**User Story:** As a developer, I want to see the current TTS status, so that I know the reading state and configuration.

#### Acceptance Criteria

1. WHILE in Reading_Mode THEN THE CLI_Interface SHALL display the current voice name and speed in the status bar
2. WHILE speech is playing THEN THE CLI_Interface SHALL display a speaking indicator with the text being read (truncated if long)
3. WHEN speech finishes THEN THE CLI_Interface SHALL update the status to idle
4. WHEN TTS_Engine initialization fails THEN THE CLI_Interface SHALL display an error message with the failure reason
