...
from tts import TTSService, TTSStatus
...
class DevEchoBackend:
    ...
    def __init__(self, socket_path: Optional[str] = None):
        ...
        # TTS service (always available, not gated by AWS config)
    ...
    def _init_tts_service(self) -> None:
        ...
        TTS is always available (not gated by AWS config).
        Sets up a status_callback that broadcasts TTSStatusMessage via IPC.

        The callback may be invoked from a daemon thread (audio playback),
        so we capture the event loop and use run_coroutine_threadsafe.
        """
        loop = asyncio.get_running_loop()

        def _tts_status_callback(status: TTSStatus):
            """Broadcast TTS status changes via IPC server."""
            ...
            asyncio.run_coroutine_threadsafe(
                self.ipc_server.send_tts_status(status_msg), loop
            )

        self._tts_service = TTSService(status_callback=_tts_status_callback)
    ...
    async def start(self) -> None:
        ...
        # Initialize TTS service (always available, not gated by AWS config)
        ...
        # Set up TTS handlers (always available)
        self.ipc_server.on_tts_speak(self._on_tts_speak)
        self.ipc_server.on_tts_stop(self._on_tts_stop)
        self.ipc_server.on_tts_set_voice(self._on_tts_set_voice)
        self.ipc_server.on_tts_preload(self._on_tts_preload)
    ...
    async def stop(self) -> None:
        ...
        # Shutdown TTS service if initialized
    ...
    async def _on_tts_speak(self, message: TTSSpeakMessage) -> None:
        """Handle TTS speak request from Swift client."""
        text_preview = f"{message.text[:50]}..." if len(message.text) > 50 else message.text
        logger.info(f"Received TTS speak request: {text_preview}")
        await self._tts_service.speak(message.text, message.language)

    async def _on_tts_stop(self, message: TTSStopMessage) -> None:
        """Handle TTS stop request from Swift client."""
        logger.info("Received TTS stop request")
        await self._tts_service.stop_speech()

    async def _on_tts_set_voice(self, message: TTSSetVoiceMessage) -> TTSVoiceConfigMessage:
        """Handle TTS set voice request from Swift client."""
        logger.info(f"Received TTS set voice request: {message.preset_name}")
        self._tts_service.set_preset(message.preset_name)
        config = self._tts_service.get_voice_config()
        presets = self._tts_service.list_presets()
        return TTSVoiceConfigMessage(
    ...
    async def _on_tts_preload(self, message: TTSPreloadMessage) -> "TTSPreloadResponseMessage":
        """Handle TTS preload request from Swift client.

        This pre-initializes the TTS engine and loads the model without speaking,
        so the first speak request has no loading delay.

        Returns TTSPreloadResponseMessage to signal completion to the client.
        """
        from ipc.protocol import TTSPreloadResponseMessage

        logger.info("Received TTS preload request, warming up model...")
        try:
            await self._tts_service.initialize()
            logger.info("TTS model preload complete")
            return TTSPreloadResponseMessage(success=True)
        except Exception as e:
            logger.error("TTS preload failed: %s", e)
            return TTSPreloadResponseMessage(success=False, error=str(e))
