...
actor IPCClient {
    ...
    // TTS status callbacks
    var onTTSStatus: (@Sendable (TTSStatusMessage) -> Void)?
    var onTTSVoiceConfig: (@Sendable (TTSVoiceConfigResponse) -> Void)?
    ...
    /// Set callback for TTS status updates from backend
    func setTTSStatusCallback(_ callback: (@Sendable (TTSStatusMessage) -> Void)?) {
        self.onTTSStatus = callback
    }

    /// Set callback for TTS voice config updates from backend
    func setTTSVoiceConfigCallback(_ callback: (@Sendable (TTSVoiceConfigResponse) -> Void)?) {
        self.onTTSVoiceConfig = callback
    }
    ...
    /// Send TTS preload request and wait for completion
    func sendTTSPreload() async throws -> TTSPreloadResponseMessage {
        let preloadMessage = TTSPreloadMessage()
        let message = preloadMessage.toIPCMessage()

        logger.debug("Sending TTS preload request...")
        try await send(message)
        logger.debug("TTS preload request sent, waiting for response...")

        // Wait for the correct response type (skip other messages like tts_status)
        while true {
            let response = try await receive()
            logger.debug("Received message type: \(response.type.rawValue)")

            // Found the expected response
            if response.type == .ttsPreloadResponse {
                guard let preloadResponse = TTSPreloadResponseMessage.fromPayload(response.payload) else {
                    logger.error("Failed to parse TTS preload response")
                    throw IPCError.decodingFailed
                }
                logger.debug("TTS preload response: success=\(preloadResponse.success)")
                return preloadResponse
            }

            // Skip other messages (e.g., tts_status during model loading)
            logger.debug("Skipping message type: \(response.type.rawValue)")
        }
    }
                    ...
                    while let newlineIndex = buffer.firstIndex(of: UInt8(ascii: "\n")) {
                        ...
                        if let jsonString = String(data: messageData, encoding: .utf8) {
                            ...
                            if let message = try? IPCMessage.fromJSON(jsonString) {
                                ...
                                else if message.type == .ttsStatus {
                                    if let status = TTSStatusMessage.fromPayload(message.payload) {
                                        logger.debug("Got TTS status: \(status.state)")
                                        onTTSStatus?(status)
                                    }
                                }
                                // Handle TTS voice config messages
                                else if message.type == .ttsVoiceConfig {
                                    if let config = TTSVoiceConfigResponse.fromPayload(message.payload) {
                                        logger.debug("Got TTS voice config: \(config.language)")
                                        onTTSVoiceConfig?(config)
                                    }
                                }
                            }
                        }
