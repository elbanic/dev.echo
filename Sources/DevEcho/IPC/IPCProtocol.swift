...
enum MessageType: String, Codable {
    ...
    case ttsSetVoice = "tts_set_voice"
    case ttsVoiceConfig = "tts_voice_config"
    case ttsPreload = "tts_preload"
    case ttsPreloadResponse = "tts_preload_response"
...
/// TTS speak request message (CLI -> Backend)
struct TTSSpeakMessage {
    let text: String
    let language: String?
    ...
    func toIPCMessage() -> IPCMessage {
        ...
        return IPCMessage(type: .ttsSpeak, payload: payload)
    }
}

/// TTS stop request message (CLI -> Backend)
struct TTSStopMessage {
    func toIPCMessage() -> IPCMessage {
        return IPCMessage(type: .ttsStop, payload: [:])
    }
}

/// TTS status update message (Backend -> CLI)
struct TTSStatusMessage {
    let state: String  // "idle", "loading_model", "generating", "playing", "stopping"
    let text: String?
    let elapsed: Double?
    let error: String?

    init(state: String, text: String? = nil, elapsed: Double? = nil, error: String? = nil) {
        self.state = state
        self.text = text
    ...
    static func fromPayload(_ payload: [String: Any]) -> TTSStatusMessage? {
        guard let state = payload["state"] as? String else {
            return nil
        }

        let text = payload["text"] as? String
        let elapsed = payload["elapsed"] as? Double
...
/// TTS set voice preset request message (CLI -> Backend)
struct TTSSetVoiceMessage {
    let presetName: String

    func toIPCMessage() -> IPCMessage {
        return IPCMessage(type: .ttsSetVoice, payload: ["preset_name": presetName])
    }
}

/// TTS voice configuration response message (Backend -> CLI)
struct TTSVoiceConfigResponse {
    let language: String
    let voiceInstruct: String
    let presetName: String?
    let availablePresets: [String]?

    init(language: String, voiceInstruct: String, presetName: String? = nil, availablePresets: [String]? = nil) {
        self.language = language
        self.voiceInstruct = voiceInstruct
        self.presetName = presetName
        self.availablePresets = availablePresets
    }

    static func fromPayload(_ payload: [String: Any]) -> TTSVoiceConfigResponse? {
        guard let language = payload["language"] as? String,
              let voiceInstruct = payload["voice_instruct"] as? String else {
            return nil
        }

        let presetName = payload["preset_name"] as? String
        let availablePresets = payload["available_presets"] as? [String]

        return TTSVoiceConfigResponse(
            language: language,
...
/// TTS preload request message (CLI -> Backend) to warm up the model
struct TTSPreloadMessage {
    func toIPCMessage() -> IPCMessage {
        return IPCMessage(type: .ttsPreload, payload: [:])
    }
}

/// TTS preload response from backend
struct TTSPreloadResponseMessage {
    let success: Bool
    let error: String?

    static func fromPayload(_ payload: [String: Any]) -> TTSPreloadResponseMessage? {
        guard let success = payload["success"] as? Bool else { return nil }
        return TTSPreloadResponseMessage(
            success: success,
