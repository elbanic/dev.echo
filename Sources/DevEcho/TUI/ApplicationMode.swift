...
enum ApplicationMode: Equatable {
    ...
    case reading              // Text-to-speech reading mode
    ...
    var availableCommandsHelp: String {
        ...
        case .command:
            ...
            /read     - Enter reading mode (text-to-speech)
        ...
        case .reading:
            return """
            /voice [name]   - List voices or set voice
            /stop           - Stop speech
            /quit           - Return to command mode
            """
        }
...
struct ApplicationModeStateMachine {
    ...
    mutating func transition(with command: Command) -> ModeTransitionResult {
        ...
        case (.command, .read):
            currentMode = .reading
            return .success(newMode: .reading)
    ...
    func targetMode(for command: Command) -> ApplicationMode? {
        ...
        case (.command, .read):
            return .reading
        case (.transcribing, .quit), (.knowledgeBaseManagement, .quit), (.reading, .quit):
            return .command
        default:
            return nil
        }
